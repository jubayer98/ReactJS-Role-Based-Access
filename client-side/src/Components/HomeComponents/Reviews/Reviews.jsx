...
import useReviewes from "../../../hooks/useReviewes";
import { useEffect } from "react";

const Reviews = () => {
  
  const [fitnessTrainerReviews] = useReviewes();

  useEffect(() => {
    // Initialize Glide after the reviews are loaded
    if (fitnessTrainerReviews.length > 0) {
      ...
      // Cleanup Glide instance when the component is unmounted
      return () => {
        slider.destroy();
      };
    }
  }, [fitnessTrainerReviews]); // Only run this effect when the reviews data changes

  return (
    <>
      <h2 className="text-center font-bold text-emerald-500 mt-5 text-2xl">REVIEWS</h2>
      <hr className="mb-5 border-emerald-500" />
      {/* Carousel with controls outside */}
      <div className="glide-04 relative w-full">
        {/* Slides */}
        <div className="overflow-hidden" data-glide-el="track">
          <ul className="whitespace-no-wrap flex-no-wrap [backface-visibility: hidden] [transform-style: preserve-3d] [touch-action: pan-Y] [will-change: transform] relative flex w-full overflow-hidden p-0">
            {fitnessTrainerReviews.map(fitnessTrainerReview => (
              <li key={fitnessTrainerReview._id}>
                  ...
                  <div className="relative p-6">
                    <figure className="relative z-10">
                      <blockquote className="p-6 text-lg leading-relaxed">
                        <p>{fitnessTrainerReview.review}</p>
                      </blockquote>
                      <figcaption className="flex items-center gap-4 p-6 pt-2 text-sm text-emerald-500">
                        <img
                          src={fitnessTrainerReview.reviewer_img_url}
                          alt={fitnessTrainerReview.reviewer}
                          title={fitnessTrainerReview.reviewer}
                          width="48"
                          height="48"
                          className="max-w-full rounded-full shrink-0"
                        />
                        <div className="flex flex-col gap-1">
                          <span className="font-bold uppercase">{fitnessTrainerReview.reviewer}</span>
                          <cite className="not-italic">
                            <a href="#" target="_blank" rel="noopener noreferrer">
                              {fitnessTrainerReview.date}
                            </a>
                          </cite>
                        </div>
                      </figcaption>
